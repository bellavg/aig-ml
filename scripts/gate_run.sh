...
#SBATCH --gpus=1
#SBATCH --job-name=progressive_gate
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=18
#SBATCH --time=48:00:00
#SBATCH --output=progressive_gate_%A.out
...
RUN_ID="progressive_gate_${TIMESTAMP}"
...
echo "Starting progressive gate masking probability training..."
echo "Run ID: $RUN_ID"
...
for MASK_PROB in "${MASK_PROBS[@]}"; do
    ...
    # Set run name
    RUN_NAME="gate_mp${MASK_PROB_DIR}"
    EXP_DIR="${MASTER_DIR}/${RUN_NAME}"

    echo "==================================================="
    echo "Starting training with gate mask probability: $MASK_PROB"
    ...
    echo "Completed training with gate mask probability: $MASK_PROB"
    echo "Best model saved at: $PRETRAINED_MODEL"
    echo "==================================================="
done

echo "Progressive gate mask training completed!"
...
for MASK_PROB in "${MASK_PROBS[@]}"; do
    ...
    if [ -f "$METRICS_FILE" ]; then
        echo "Gate mask probability: $MASK_PROB" >> "${MASTER_DIR}/summary.txt"
        ...
        echo "No results found for gate mask probability: $MASK_PROB" >> "${MASTER_DIR}/summary.txt"
    fi
done
